...
object GithubUrl {
  ...
  def apiSearch(q: String) = apiBase + "legacy/repos/search/" + q
...
object Contributor {
  def apply(c: Contributor, total: Int) = {
    new Contributor(c.login, c.avatar_url, c.contributions, Some(total))
  }
}

case class Contributor(login: String, avatar_url: String, contributions: Int = 0, totalContribs: Option[Int] = None) {
